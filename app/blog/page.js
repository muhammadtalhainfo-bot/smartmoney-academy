...
export default function BlogPage() {
  ...
  const rest = active === 'All' ? POSTS.slice(1) : filtered;
          ...
          <div style={{ textAlign: 'center', maxWidth: '600px' }}>
            ...
            {!subscribed ? (
              <div style={{ display: 'flex', gap: '8px', maxWidth: '400px', margin: '0 auto' }}>
                <input className="auth-input" type="email" placeholder="your@email.com" value={email} onChange={e => setEmail(e.target.value)} style={{ flex: 1 }} />
                <button onClick={() => email.includes('@') && setSubscribed(true)} style={{ padding: '10px 20px', background: 'linear-gradient(135deg,#D4A843,#8A6B28)', border: 'none', borderRadius: '8px', color: 'black', fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '0.1em', cursor: 'pointer', fontWeight: 600, whiteSpace: 'nowrap' }}>
                  SUBSCRIBE
                </button>
              </div>
            ) : (
              <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '12px', color: '#34D399' }}>✓ You're subscribed!</div>
            )}
          </div>
        </div>
      </section>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '48px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
          <h2 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '24px', color: 'white', letterSpacing: '0.05em' }}>
            {active === 'All' ? 'ALL ARTICLES' : active.toUpperCase()}
          </h2>
