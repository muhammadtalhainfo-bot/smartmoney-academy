...
const BANNERS = {
  ...
  '01': ({ w, h }) => (  // Market Structure — HH/HL trend
    <>
      ...
      {/* BOS label */}
      <rect x={130} y={h-100} width={44} height={16} rx="3" fill="rgba(212,168,67,0.1)" stroke={GOLD} strokeWidth="0.7" strokeOpacity="0.4" />
  ...
  '04': ({ w, h }) => (  // Order Blocks — OB zone + price return
    <>
      <GridLines w={w} h={h} spacing={36} opacity={0.03} />
      {/* OB zone */}
      <rect x={20} y={55} width={70} height={35} rx="3" fill="rgba(212,168,67,0.1)" stroke={GOLD} strokeWidth="1.2" strokeOpacity="0.7" />
  ...
  '10': ({ w, h }) => (  // SMT Divergence — two correlated pairs
    <>
      ...
      {/* Divergence zone */}
      <rect x={155} y={42} width={60} height={40} rx="3" fill="rgba(212,168,67,0.08)" stroke={GOLD} strokeWidth="0.8" strokeOpacity="0.5" strokeDasharray="3,3" />
  ...
  '13': ({ w, h }) => (  // SMC — CHoCH / supply demand
    <>
      ...
      <rect x={w/2-20} y={h/2-10} width={40} height={20} rx="3" fill="rgba(212,168,67,0.1)" stroke={GOLD} strokeWidth="0.8" strokeOpacity="0.5" />
  ...
  'what-is-ict-trading': ({ w, h }) => (
    <>
      ...
      <polyline points={`18,${h*0.6} 55,${h*0.5} 95,${h*0.55} 135,${h*0.38} 175,${h*0.42} 215,${h*0.3} ${w-20},${h*0.25}`}
        fill="none" stroke={GOLD} strokeWidth="1.6" strokeOpacity="0.7" />
      <rect x={20} y={h*0.28} width={55} height={18} rx="3" fill="rgba(212,168,67,0.1)" stroke={GOLD} strokeWidth="0.8" strokeOpacity="0.5" />
      ...
      {label && (
        <g>
          <rect
            ...
            stroke={levelColor?.border || 'rgba(212,168,67,0.3)'}
            strokeWidth="0.8"
          />
          <text
